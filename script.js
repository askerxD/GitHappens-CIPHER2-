...
function renderIssues() {
    ...
    el.innerHTML = issues.length ? issues.map(i=> `
        <div class="issue-item">
            <strong>${i.resource} — ${i.type}</strong>
            <p>${i.desc}</p>
            <span>${i.time}</span>
        </div>
    `).join('') : '<div style="text-align:center;padding:40px;color:var(--text-muted);">No issues reported</div>';
}
...
function renderStaffActivity() {
    ...
    if (!tb) return;
    tb.innerHTML = staffActivity.map(a=> `
        <tr>
            <td>${a.time}</td>
            <td>${a.student}</td>
            <td>${a.resource}</td>
            <td><span class="badge badge-${a.action==='Check In'?'available':'occupied'}">${a.action}</span></td>
...
function restrictUser(s) { 
    openModal('Restrict User', `<p>Block new bookings for <strong>${s}</strong>?</p>`, 
        `<button class="btn btn-secondary" onclick="closeModal()">Cancel</button>
         <button class="btn btn-danger" onclick="closeModal();showToast('${s} restricted','danger')">Restrict</button>` 
    ); 
}
...
function renderCurrentLoans() {
    ...
    if (!tb) return;
    tb.innerHTML = currentLoans.map(l=> `
        <tr>
            <td>${l.student}</td>
            <td>${l.resource}</td>
            <td>${l.due}</td>
            <td><span class="badge badge-${l.status==='ok'?'available':'overdue'}">${l.status==='ok'?'On Time':'Overdue'}</span></td>
        </tr>
    `).join('');
}
...
function renderBarChart(id, data) {
    ...
    el.innerHTML = data.map(d=>`
        <div class="chart-bar-row">
            ...
            <div class="chart-bar-track"><div class="chart-bar-fill" style="width:${d.pct}%;background:${d.color};">${d.pct >15?d.val:''}</div></div>
            <div class="chart-bar-val">${d.val}</div>
        </div>
    `).join('');
}
